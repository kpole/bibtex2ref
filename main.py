...
class MergeNameParts(BlockMiddleware):
    def transform_entry(self, entry, *args, **kwargs):
        MAX_AUTHOR_COUNT = 6
        MIDDLE_NAME_ABBREVIATION = 'C.'
        
        convert_names = []
        for name_part in entry['author'][:MAX_AUTHOR_COUNT]:
            convert_name = []
            convert_name.append(name_part.first[0])
            if len(name_part.first + name_part.von + name_part.last + name_part.jr) > 2:
                convert_name.append(MIDDLE_NAME_ABBREVIATION)
            convert_name.append(name_part.last[0])
            convert_names.append(' '.join(convert_name))
